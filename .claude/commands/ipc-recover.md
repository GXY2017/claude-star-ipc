...
1. **确定角色**(按优先级,**别盲信旧注入**):
   ...
   - ⚠️ **若你发现自己当前正以"错误角色"挂着盯哨**(如本该是 B 却在跑 `watch --me D`):先 `TaskStop` 那个错角色的 Monitor,再按正确角色走第 3–4 步。"一个信箱一个盯哨"是指"每个信箱同时只一个",**不是"不许改正错角色"**——切到对的信箱是应该的。幂等"已恢复"判断要基于**正确角色**,不是基于你碰巧在跑的那个。
